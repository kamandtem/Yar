    ...
    <>
          ...
          return (
            <button
              ...
              className={`icon-only rounded-xl font-bold transition-all aspect-square ${compact ? 'text-xs rounded-lg' : 'text-sm'} ${
                isSelected
                  ? 'bg-[oklch(52%_0.14_330)] text-white'
                  : isToday
                    ? 'bg-[oklch(93%_0.04_330)] dark:bg-pink-950/30 text-[oklch(46%_0.13_330)] dark:text-pink-300 border border-[oklch(78%_0.08_330)]'
                    : disabled
      ...
      {allowPast && !inline && (
        <button
          ...
          className="w-full py-2 rounded-xl bg-slate-50 dark:bg-slate-800 text-sm font-bold text-[oklch(48%_0.13_330)] dark:text-pink-300"
        >
          امروز
        </button>
      )}
    ...
    <div className={inline ? 'space-y-2' : 'space-y-2'}>
      ...
      {!inline && (
        ...
        >
          ...
          <Calendar className="w-5 h-5 text-[oklch(52%_0.14_330)] shrink-0" />
        </button>
      )}

      {isOpen && inline && (
        <div
          className={`rounded-2xl bg-white dark:bg-slate-900 border border-[oklch(90%_0.025_330)] dark:border-slate-700 shadow-lg ${compact ? 'p-2 space-y-1' : 'p-3 space-y-3'}`}
        >
          {calendarBody}
        </div>
      )}
      ...
      {isOpen && !inline && (
        ...
        >
          <div
            className="w-full max-w-[19rem] p-3 rounded-2xl bg-white dark:bg-slate-900 border border-[oklch(90%_0.025_330)] dark:border-slate-700 shadow-2xl space-y-3"
