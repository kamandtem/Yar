...
export function buildPersonalNotifications(args: { phase?: MenstrualPhase | null; inPms?: boolean; temperature?: number | null; moodInsight: MoodInsight }) {
  ...
  const activeQuote = currentHour < 12 ? quotes.morning : quotes.evening;
  ...
  return [
    { id:`daily-quote-${quotes.dateKey}-${currentHour < 12 ? 'day' : 'night'}`, title:'', body:activeQuote, tone:(currentHour < 12 ? 'warm' : 'calm') as const, target:'home' as const },
    ...items
  ];
}
